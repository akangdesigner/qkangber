// 發布 30-vibe-coding 到 posts 分頁（Vibe Coding pillar / hub）。
// 用法：node scripts/publish-vibe-coding.mjs [--write] [--update]
//   預設 dry-run；--write 才寫入；--update 覆蓋既有同 slug 那列（否則 append 新列）。
// 官網版處理：去 h1、紅 #c0392b → 琥珀 #fbbf24（但跳過 <svg> 區塊，示意圖配色不轉）。
// 三張概念圖是 inline SVG（非 <img>）；唯一 <img> 是封面，src 換成 ImgBB 網址。
import fs from 'fs'
...
const TITLE = 'Vibe Coding 是什麼？用 AI 寫程式的完整運作方式與學習順序'
const DATE = '2026/08/01'
const TAGS = 'Vibe Coding,AI 寫程式,AI 工具,學習順序,程式入門'
const EXCERPT = 'Vibe Coding 是什麼？拆解它把一句話變成程式碼的運作原理、能做出什麼、該用哪類工具，並排出四階段學習順序與三種最常見的翻車狀況與擋法。'
const CATEGORY = 'AI 軟體開發'     // M 主分類（dry-run 會印既有分類供核對）
const SUBCATEGORY = ''            // O 副分類已退役，留空
const COVER = 'https://i.ibb.co/WWFdjm5t/cover.webp'

// --- 內文轉換 ---
const raw = fs.readFileSync('blog-drafts/30-vibe-coding/30-vibe-coding.html', 'utf8')
...
console.log(`內文長度=${content.length}｜<img>=${leftover.imgs}（應1：封面）｜inline <svg>=${leftover.svgs}（應3）｜<table>=${leftover.tables}（應2）`)
