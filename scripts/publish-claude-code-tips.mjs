// 發布 32-claude-code-beginner-tips 到 posts 分頁（Claude Code 集群第三篇）。
// 用法：node scripts/publish-claude-code-tips.mjs [--write] [--update]
//   預設 dry-run；--write 才寫入；--update 覆蓋既有同 slug 那列（否則 append 新列）。
// 官網版處理：去 h1、紅 #c0392b → 琥珀 #fbbf24（但跳過 <svg> 區塊，示意圖配色不轉）。
// 兩張 <img>（封面＋權限模式截圖）換成 ImgBB 網址；兩張概念圖是 inline SVG 不動。
import fs from 'fs'
...
const SLUG = 'claude-code-tips'
const TITLE = 'Claude Code 怎麼用？新手必學的 10 個技巧 讓 AI 照你的規則做事'
const DATE = '2026/08/05'
const TAGS = 'Claude Code,AI 寫程式,CLAUDE.md,權限模式,提示詞'
const EXCERPT = 'Claude Code 怎麼用？新手必學 10 個技巧：CLAUDE.md 怎麼寫、權限白名單、Hooks、提示詞給檔名行號，加四種權限模式怎麼選，讓 AI 照你的規則做事。'
const CATEGORY = 'AI 軟體開發'     // M 主分類（dry-run 會印既有分類供核對）
const SUBCATEGORY = ''            // O 副分類已退役，留空
const COVER = 'https://i.ibb.co/4vbrCH3/cover.webp'
const SHOT_SHIFT_TAB = 'https://i.ibb.co/WvT6hXry/screenshot-shift-tab.webp'

// --- 內文轉換 ---
const raw = fs.readFileSync('blog-drafts/32-claude-code-beginner-tips/32-claude-code-beginner-tips.html', 'utf8')
...
content = content.replace(/src="images\/screenshot-shift-tab\.jpg"/gi, `src="${SHOT_SHIFT_TAB}"`)

// 紅 → 琥珀，但先把 <svg> 區塊抽出保護（示意圖內配色不轉）
const svgBlocks = []
...
console.log(`內文長度=${content.length}｜<img>=${leftover.imgs}（應2：封面＋截圖）｜inline <svg>=${leftover.svgs}（應2）｜<table>=${leftover.tables}（應1）`)
console.log(`紅字：body 殘留=${leftover.redBody}（應0）｜SVG 內保留=${leftover.redSvgKept}（原 SVG 有 ${redInSvg}，應原樣保留）`)
console.log(`本地圖殘留=${leftover.localImg}（應0）｜CTA 連結=${leftover.serviceCta}（應≥2）`)
