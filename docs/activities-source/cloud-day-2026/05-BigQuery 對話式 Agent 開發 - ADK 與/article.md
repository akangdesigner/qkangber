# BigQuery 對話式 Agent 開發 - ADK 與 MCP 的實戰整合
*Cloud Day 2026・2026-07-09*
> 讓 Agent 查詢 BigQuery 有五種做法，從免寫程式到完全客製都有；91APP 用「資料定義治理（Looker）＋確定性檢查用程式碼、不信任 Agent 判斷」的方式，把資料 Agent 做到能給兩萬多客戶安全使用的規模。
*下午・加值產業轉型與智慧代理*

**Simon Wang**・Google Cloud 台灣解決方案架構師
**Happy Lee 李昆謀**・91APP 產品長
