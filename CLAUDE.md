...
- 部落格內頁的 meta title **不加**品牌後綴「— Q kangber」（用 `title: { absolute }` 繞過 layout 樣板），字元全部留給關鍵字；其餘頁面仍沿用全站樣板後綴
- meta title 建議中文控制在 25–30 字內，核心關鍵字與問句放最前面 15 字內。title 被截斷不是單純字數問題——Google 常依查詢字詞、頁面 H1 自行改寫 SERP 標題顯示，字數壓短只能降低風險、不保證不被動；title 要跟頁面 H1 語意一致，避免 Google 改寫後跟內容對不上
- 圖片必須有描述性 alt 文字
